...
function SubmitReferralForm({ teacher = {}, onCancel, onSuccess }) {
    ...
    // Predefined categories and their corresponding violations
    const categories = [
    ...
    // This useEffect hook updates the `violations` state whenever the category changes.
    useEffect(() => {
        const selectedViolations = allViolations[referral.counselingTypeCategory] || [];
        setViolations(selectedViolations);
        // Reset violation when category changes to prevent an invalid selection
        setReferral(prev => ({ ...prev, violation: '' }));
    }, [referral.counselingTypeCategory]);


    // Handles changes to form inputs (text, radio, checkbox, select).
    ...
    // New function for the dynamic violation input
    ...
    // New function for the category dropdown
    const handleCategoryDropDown = () => (
        <div>
            <label htmlFor="counselingTypeCategory" className="block text-gray-700 font-medium mb-1">Counseling Type/Category:</label>
            <select
