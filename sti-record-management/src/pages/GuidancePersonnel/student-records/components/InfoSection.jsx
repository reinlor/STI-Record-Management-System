...
const InfoSection = ({ infoType, student = {}, isEditing, onFieldChange }) => {
    const sectionDef = fieldDefinitions[infoType];
    ...
    const renderField = (fieldDef) => {
        // Hide editOnly fields when not editing, and vice-versa
        if (isEditing && fieldDef.readOnly) return null;
        if (!isEditing && fieldDef.editOnly) return null;

        // Special case for medicalCert
        if (fieldDef.key === "medicalCert") {
            return (
                <div key={fieldDef.key} className="flex flex-col col-span-3">
                    {renderMedicalCertField(fieldDef)}
                </div>
            );
        }

        // Special case for Last Doctor Visit
    ...
    if (!sectionDef) {
        return <div>Invalid info type: {infoType}</div>;
    }

    return (
        <div className="space-y-3">
            <h3 className="text-2xl font-bold text-[#0172bd] border-b pb-2">
                {sectionDef.title}
            </h3>

            {/* Render top-level fields if they exist, wrapped in a box */}
            {sectionDef.fields && (
                <div className="bg-gray-100 p-6 rounded-xl shadow-md border border-gray-100">
                        ...
                        {sectionDef.fields
                            .filter(
                                (field) =>
                                    !(
                                        infoType === "health" &&
                                        (field.key === "hospitalized" || field.key === "reason")
                                    )
                            )
                            .map(renderField)}
                    </div>
            ...
            {/* Render subsections, each in its own styled box */}
            {sectionDef.subsections &&
                sectionDef.subsections.map((subsection) => (
                    <div key={subsection.title} className="bg-gray-100 p-6 rounded-xl shadow-md border border-gray-200 space-y-4">
                        <h4 className="text-xl font-semibold text-[#0172bd]">
                            {subsection.title}
                        </h4>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-6 pt-2">
