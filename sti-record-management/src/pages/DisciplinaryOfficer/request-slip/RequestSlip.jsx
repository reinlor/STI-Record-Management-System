...
function RequestSlip() {
  ...
  const requestTable = allSlipData.map((slips) => (
    <tr key={slips._id}>
      <td>{slips.name}</td>
      <td>{slips.sid}</td>
      ...
      <td>{slips.status}</td>
      <td>{slips.reason}</td>
      <td>{slips.attachmentCount}</td>
      <td>
        <button onClick={() => setDisplay(!display)}>Open</button>
      </td>
    </tr>
  ));
  ...
  return (
    <div>
      ...
      <button onClick={() => navigate("/disciplinary/request-slip-history")}>
        History
      </button>
